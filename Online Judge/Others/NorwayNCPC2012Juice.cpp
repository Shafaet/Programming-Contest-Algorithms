...
int need[1002];
int dp[1002][102][102];
bool vis[1002][102][102];
int call(int u,int e,int power)
{
		if(e>=sz(g[u])) return power>=need[u];
		if(vis[u][e][power]) return dp[u][e][power];
		vis[u][e][power]=1;
		int ans=0;
		for(int d=0;d<=power and d<=g[u][e].second;d++)
		{
			int v=g[u][e].first;
			int ret=call(v,0,d)+call(u,e+1,power-d);
			ans=max(ans,ret);
		}
		return dp[u][e][power]=ans;
}
int main(){

	//READ("data/juice/juice.02.in");
