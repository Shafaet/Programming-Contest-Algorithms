...
int cr=0,cb=0,comp=0;
int imp=0;
vector<int>sav[mx];
void Bicolor(int source)
{
	sav[comp].pb(source);
    for(int i=0;i<(int)gg[source].size();i++)
    {
        int curr=gg[source][i];
        if(color[curr]==color[source]) {imp=1;return;}
        else if(!color[curr] && !visited[curr])
        {
            if(color[source]==RED) {color[curr]=BLUE;cb++;}
            else {color[curr]=RED;cr++;}
            visited[curr]=1;
            Bicolor(curr);
...
int call(int idx,int sum)
{
	...
	int ret1=call(idx+1,sum+red[idx]);
	int ret2=call(idx+1,sum+blue[idx]);
	
	if(ret1==1) dir[idx]=RED;
	 if(ret2==1) dir[idx]=BLUE;
...
{	
	...
	else
	{
		...
		{	
			...
			{
				int u=sav[i][j];
				if(color[u]==dir[i]) {printf("%d ",u);take[u]=1;}
			}

		}
		puts("");
		for(int i=1;i<=n+n;i++)if(!take[i]) cout<<i<<" ";
		puts("");
